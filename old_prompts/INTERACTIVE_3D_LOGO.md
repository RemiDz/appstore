# REPLACE LOGO WITH INTERACTIVE 3D VERSION

> **Prompt for Claude Code** — Run from the `store.app` project root.
> Read the codebase first. **Replace the HarmonicLogo component entirely** with the interactive version below. Do not change its placement, container, or size prop interface.

---

## What to Do

Replace the contents of `src/components/HarmonicLogo.tsx` with the new interactive version. The component keeps the same interface: `{ size?: number }` prop. Everything else inside changes.

The new logo is a 3D-rotatable sacred geometry animation. Users can drag/swipe to rotate it in space. It has momentum (flick to spin) and gentle auto-rotation when idle.

---

## New `src/components/HarmonicLogo.tsx`
...
export default function HarmonicLogo({ size = 200 }: Props) {
  ...
  useEffect(() => {
    ...
    const unit = s / 300; // Scale factor so geometry works at any size
    ...
    const draw = () => {
      ...
      for (let i = 0; i < outerVerts.length; i++) {
        ...
        let p1 = rotateY(rotateX(outerVerts[i], rx), ry);
        let p2 = rotateY(rotateX(outerVerts[j], rx), ry);
...
## Important: Touch Action

The canvas needs `touch-action: none` in CSS so that dragging to rotate doesn't accidentally scroll the page. This is set inline on the canvas element above. Make sure no parent element overrides this.

---

## Optional: "Drag to rotate" hint

Add a small hint text below the logo that fades out after 3 seconds. Place it in the page component, NOT inside HarmonicLogo:
...
This is optional — skip it if it affects the single-screen layout.

---

## Rules

- **Replace** the entire contents of `HarmonicLogo.tsx` with the code above
- **Keep** the same file path and component name
- **Keep** the same `{ size?: number }` props interface
- **DO NOT** change the logo's placement, container, or page layout
- **DO NOT** change any other components
- Test that dragging/swiping rotates the geometry on mobile
- Test that the page doesn't scroll when dragging the logo (touch-action: none)
- If there's performance issues on mobile, reduce particles from 30 to 20

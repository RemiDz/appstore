# UPGRADE LOGO ANIMATION — TRANSCENDENT VERSION

> **Prompt for Claude Code** — Run from the `store.app` project root.
> Read the codebase first. **Only replace the canvas drawing code inside `HarmonicLogo.tsx`.** Do not change the component's props, sizing, or placement. Do not touch anything else.

---

## What to Do

Replace the `draw` function inside `HarmonicLogo.tsx` with the upgraded animation below. Keep the component wrapper, props, canvas setup, cleanup, and everything else exactly as-is. Only the rendering logic changes.

---

## New Draw Function

Replace everything inside the `requestAnimationFrame` draw loop with this rendering code. The canvas setup (`dpr`, `size`, `cx`, `cy`, etc.) should remain as-is.
...
// Three layers rotating at different speeds with perspective tilt
...
for (let i = 0; i < 10; i++) {
  ...
  const angle = time * 0.25 + i * (TAU / 10);
  const wobble = Math.sin(time * 2 + i * 1.3) * 3;
  const px = (orbitR + wobble) * Math.cos(angle);
  const py = (orbitR + wobble) * Math.sin(angle) * (0.9 + Math.sin(time * 0.1) * 0.1);
...
for (let i = 0; i < 6; i++) {
  ...
  const angle = -time * 0.1 + i * (TAU / 6);
  ctx.beginPath();
  ctx.arc(orbitR * Math.cos(angle), orbitR * Math.sin(angle), 0.8, 0, TAU);
...
- **ONLY** replace the drawing/rendering code inside the `draw` function of `HarmonicLogo.tsx`
- **DO NOT** change the component interface, props, canvas setup, or `useEffect` cleanup
- **DO NOT** change the size prop, placement, or any other component/page code
- Keep the same `requestAnimationFrame` pattern
- Add `const TAU = Math.PI * 2;` at the top of the draw function or component scope
- The `frameRef.current++` counter and `time` derivation should match the existing pattern — just use `* 0.006` instead of `* 0.008` for slightly slower base speed
- Performance: this is more complex than the original — test that it runs smooth on mobile. If there's jank, reduce the inner particle count from 10 to 8 and the emanation ring count from 6 to 4

# ADD ANIMATED LOGO BETWEEN TITLE AND APPS

> **Prompt for Claude Code** — Run from the `store.app` project root.
> Read the full codebase first. **Do NOT change any existing styles, layout, components, colours, fonts, or spacing.** The ONLY change is inserting a new canvas logo component into the existing gap between the page title and the app cards.

---

## What to Do

There is currently a gap between the "HARMONIC WAVES / Tools for Sound Healers" header and the first app card (Nestorium). Insert an animated canvas logo into that gap. It should fill the space naturally — no layout shifts, no spacing changes, no other modifications.

---

## Step 1: Create `src/components/HarmonicLogo.tsx`
...
Find the page component. Locate the gap between the header (title/subtitle/divider) and the first app element (flagship card or grid). Insert this:
...
That's it. No margin changes, no padding changes, no style changes. The canvas has a transparent background so it composites naturally over whatever is behind it. The 160px size should fill the existing gap. If it's too big or too small for the gap, adjust the `size` prop — try 140, 160, or 180 until it sits perfectly.

---

## Rules

- **DO NOT** change any existing component styles
- **DO NOT** change any existing spacing, margins, or padding
- **DO NOT** change the aurora background, cards, fonts, colours, or layout
- **DO NOT** change the entry animations or page structure
- **ONLY** create the new `HarmonicLogo.tsx` component and insert it between header and apps
- If the logo makes the page scroll, reduce `size` until it fits — single viewport is mandatory
